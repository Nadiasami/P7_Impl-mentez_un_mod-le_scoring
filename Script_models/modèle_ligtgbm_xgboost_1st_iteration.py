...
def objective_xgboost(params, train_df, test_df, num_folds= 10, stratified=False, debug=False):
...
def found_best_model_xgboost(train_df, test_df):
    ...
    # Saving best results
    best_params = fmin(
        fn=lambda params: objective_xgboost(params, train_df, test_df, num_folds= 10, stratified=False, debug=False),
        space=search_space,
        algo=tpe.suggest,
        max_evals=10,
        trials=Trials(),
...
def main_flow() -> None:
    ...
    mlflow.set_experiment("modèle_lgbm_xgb")
